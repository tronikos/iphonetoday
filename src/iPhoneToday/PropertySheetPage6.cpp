...
SpecialIconSettings batt, dow, dom, clck, vol, mem;
	...
	case WM_INITDIALOG:
		{
			...
			if (configuracion != NULL) {
				...
				memcpy(&mem,  &configuracion->mem,  sizeof(SpecialIconSettings));
...
BOOL SaveConfiguration6(HWND hDlg)
{
	...
	if (!check(hDlg, &mem)) return FALSE;
	...
	memcpy(&configuracion->mem,  &mem,  sizeof(SpecialIconSettings));
